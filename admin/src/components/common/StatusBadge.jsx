...
interface StatusBadgeProps {
    ...
    type?: 'order' | 'payment' | 'food' | 'user' | 'delivery' | 'review' | 'custom';
    variant?: 'default' | 'outline' | 'ghost' | 'dot' | 'pill';
    size?: 'sm' | 'md' | 'lg';
    showIcon?: boolean;
    showLabel?: boolean;
    animated?: boolean;
...
const StatusBadge: React.FC<StatusBadgeProps> = ({
    ...
    const getOrderStatusConfig = (value: string): StatusConfig => {
        const config: Record<string, StatusConfig> = {
            ...
            delivered: {
                ...
                actions: [
                    { label: 'Reorder', onClick: () => {}, variant: 'primary' },
                    { label: 'Review', onClick: () => {}, variant: 'secondary' }
                ]
            },
            cancelled: {
                color: 'red',
        ...
        return config[value] || { color: 'gray', label: value || 'Unknown', icon: <FiInfo className="w-3 h-3" /> };
    };

    // Payment status configurations
    const getPaymentStatusConfig = (value: string): StatusConfig => {
        const config: Record<string, StatusConfig> = {
            pending: {
                color: 'yellow',
                label: 'Pending',
    ...
    // Food status configurations
    const getFoodStatusConfig = (value: boolean): StatusConfig => {
        return value 
            ? { color: 'green', label: 'Available', icon: <FiCheckCircle className="w-3 h-3" /> }
            : { color: 'red', label: 'Unavailable', icon: <FiXCircle className="w-3 h-3" /> };
    };

    // User status configurations
    const getUserStatusConfig = (value: string): StatusConfig => {
        const config: Record<string, StatusConfig> = {
            active: {
                color: 'green',
                label: 'Active',
    ...
    const getDeliveryStatusConfig = (value: string): StatusConfig => {
        const config: Record<string, StatusConfig> = {
            assigned: {
                color: 'blue',
    ...
    const getReviewStatusConfig = (value: string): StatusConfig => {
        const config: Record<string, StatusConfig> = {
            pending: {
                color: 'yellow',
    ...
    const getConfig = (): StatusConfig => {
        ...
        switch (type) {
            case 'order':
                return getOrderStatusConfig(status as string);
            case 'payment':
                return getPaymentStatusConfig(status as string);
            case 'food':
                return getFoodStatusConfig(status as boolean);
            case 'user':
                return getUserStatusConfig(status as string);
            case 'delivery':
                return getDeliveryStatusConfig(status as string);
            case 'review':
                return getReviewStatusConfig(status as string);
            default:
    ...
    const getVariantClasses = () => {
        const color = config.color as keyof typeof colorClasses;
    ...
    if (variant === 'dot') {
        return (
            <div className="flex items-center gap-1.5">
                <motion.span
                    className={`block rounded-full ${sizeClasses[size].dot} ${colorClasses[config.color as keyof typeof colorClasses]?.dot}`}
                    animate={pulsating ? {
                        scale: [1, 1.5, 1],
                        opacity: [1, 0.5, 1]
                    } : {}}
    ...
    return (
        <div className="relative inline-block">
            ...
            >
                ...
                {interactive && config.actions && config.actions.length > 0 && (
                    <motion.button
            ...
            <AnimatePresence>
                {withDescription && isHovered && config.description && (
                    <motion.div
                        initial={{ opacity: 0, y: 5 }}
                        animate={{ opacity: 1, y: 0 }}
                        exit={{ opacity: 0, y: 5 }}
