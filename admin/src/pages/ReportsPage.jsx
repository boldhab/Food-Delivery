...
import {
    ...
    ResponsiveContainer, Cell, PolarGrid, PolarAngleAxis,
...
interface ReportData {
    ...
    userGrowth: Array<{ date: string; customers: number; drivers: number }>;
    ...
    customerRetention: Array<{ month: string; new: number; returning: number }>;
    driverPerformance: Array<{ driver: string; deliveries: number; rating: number; earnings: number }>;
    inventoryAlerts: Array<{ item: string; current: number; threshold: number }>;
    revenueByLocation: Array<{ location: string; revenue: number; orders: number }>;
...
const ReportsPage: React.FC = () => {
    ...
    const [scheduledReports, setScheduledReports] = useState<ScheduledReport[]>([
        {
            ...
            format: 'pdf',
            lastSent: format(subDays(new Date(), 1), 'yyyy-MM-dd'),
            nextSend: format(new Date(), 'yyyy-MM-dd')
        },
        {
            id: '2',
            ...
            format: 'excel',
            lastSent: format(subDays(new Date(), 7), 'yyyy-MM-dd'),
            nextSend: format(new Date(), 'yyyy-MM-dd')
        }
    ]);
    ...
    const exportAsImage = async () => {
        ...
        const a = document.createElement('a');
        a.href = url;
        a.download = `${activeReport}-report-${format(new Date(), 'yyyy-MM-dd')}.png`;
        a.click();
    };
                            ...
                            <motion.div variants={cardVariants} className="bg-white dark:bg-slate-800 rounded-xl p-6 border border-slate-200 dark:border-slate-700">
                                <div className="flex items-center justify-between">
                                    <div>
                                        ...
                                        <p className="text-2xl font-bold text-slate-900 dark:text-white">
                                            ${(reportData.salesData?.reduce((sum, d) => sum + d.revenue, 0) / 
                                               reportData.salesData?.reduce((sum, d) => sum + d.orders, 0)).toFixed(2)}
                                        </p>
                                    </div>
                                        ...
                                        <button
                                            ...
                                            className={`p-2 rounded-lg transition-colors ${
                                                selectedChartType === option.value
                                                    ...
                                                    : 'bg-slate-100 dark:bg-slate-700 text-slate-500 hover:text-orange-500'
                                            }`}
                                        >
                    ...
                    <div className="space-y-6">
                        <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
                            ...
                            <motion.div variants={itemVariants} className="bg-white dark:bg-slate-800 rounded-xl p-6 border border-slate-200 dark:border-slate-700">
                                ...
                                <div className="space-y-4">
                                    {reportData.inventoryAlerts?.map((item) => (
                                        <div key={item.item} className="flex items-center justify-between p-3 bg-red-50 dark:bg-red-900/20 rounded-lg">
                                            <div>
                            ...
                            <motion.div variants={itemVariants} className="bg-white dark:bg-slate-800 rounded-xl p-6 border border-slate-200 dark:border-slate-700">
                                ...
                                <div className="space-y-4">
                                    {reportData.driverPerformance?.map((driver) => (
                                        <div key={driver.driver} className="flex items-center justify-between p-3 bg-slate-50 dark:bg-slate-900/50 rounded-lg">
                                            <div>
    ...
    return (
        ...
        >
                    ...
                    <button
                        ...
                        className={`
                            ...
                            ${activeReport === type.id
                                ? `bg-${type.color}-500 text-white shadow-lg shadow-${type.color}-500/25`
            ...
            {scheduledReports.length > 0 && (
                <motion.div variants={itemVariants} className="bg-white dark:bg-slate-800 rounded-2xl p-6 border border-slate-200 dark:border-slate-700">
